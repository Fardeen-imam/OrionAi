...
function getQuotaMessage(error: unknown): string | null {
  ...
  const status =
    (error as { status?: string; code?: string; message?: string })?.status ??
    (error as { status?: string; code?: string; message?: string })?.code;
  ...
  if (
    ...
    return "Gemini API quota has been exhausted for this free tier. Please try again shortly or switch to a paid plan if available.";
  }
...
async function generateWithFallback(
  ...
  for (const model of models) {
    try {
      ...
      lastError = error;
      const quotaMessage = getQuotaMessage(error);
      if (quotaMessage) {
        throw new Error(quotaMessage);
      }

      const status =
        (error as { status?: string; code?: string; message?: string })
          ?.status ??
        (error as { status?: string; code?: string; message?: string })?.code;
      const message = String((error as { message?: string })?.message ?? "");
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const geminiStream = await generateWithFallback(contents);
        ...
        const quotaMessage = getQuotaMessage(err);
        enqueue(
          sseEvent("error", {
            message: quotaMessage ?? "Something went wrong. Please try again.",
          }),
        );
      } finally {
        controller.close();
