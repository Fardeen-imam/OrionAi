...
export const SectionLabel = ({ children }: { children: React.ReactNode }) => {
  return (
    <p className="inline-flex items-center gap-2 text-xs font-semibold text-blue-400 tracking-[0.14em] upppercase mb-4">
      {children}
...
export const SectionHeading = ({
  ...
  return (
    <h2 className="font-serif text-[clamp(2rem,_4vw,_3rem)] leading-[1.1] tracking-tight">
      <GrayTitle>{gray}</GrayTitle>
      <br />
      {/* 🟢 Added a larger fluid text size clamp specifically for the blue text */}
      <span className="text-[clamp(2.5rem,_5vw,_4rem)] block mt-2">
        <BlueTitle>{blue}</BlueTitle>
      </span>
    </h2>
  );
