...
function normalizeSandpackPath(path: string): string {
  ...
  if (cleaned.startsWith("/src/")) return cleaned;
...
function getFriendlyFallbackFiles(message: string | null) {
  const userMessage =
    message ??
    "We’re taking a short breather while the AI generator catches up.";
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ margin: 0, fontSize: 12, letterSpacing: "0.24em", textTransform: "uppercase", color: "#7dd3fc" }}>
          Preview ready
        </p>
        <h1 style={{ margin: "12px 0 10px", fontSize: 32, color: "#f8fafc" }}>Hello world</h1>
        <p style={{ margin: 0, fontSize: 15, lineHeight: 1.7, color: "rgba(248,250,252,0.8)" }}>
          ${userMessage}
        </p>
      </div>
    </div>
...
function normalizeSandpackFiles(
  ...
  fallbackFiles: Record<string, { code: string }> = PLACEHOLDER_FILES,
) {
  const source = files ?? fallbackFiles;
  const normalized: Record<string, { code: string }> = {};

  Object.entries(source).forEach(([path, file]) => {
    const code =
      typeof file === "object" && file !== null && "code" in file
        ? file.code
        : "";
    normalized[normalizeSandpackPath(path)] = { code };
  });

  if (!normalized["/src/index.js"] && !normalized["/src/main.js"]) {
...
function SandpackInner({
  ...
  useEffect(() => {
    const normalizedFiles = normalizeSandpackFiles(
      fileData?.files,
      generationError
        ? getFriendlyFallbackFiles(generationError)
        : PLACEHOLDER_FILES,
    );
    ...
    prevFilesRef.current = normalizedFiles;
