  ...
  const handleGenerate = useCallback(
    async (prompt: string, imageUrl?: string) => {
      ...
      try {
        ...
        if (res.status === 402) {
          const message =
            "Generation is temporarily unavailable because you do not have enough credits.";
        ...
        if (res.status === 429) {
          const message =
            "We hit a temporary AI limit. Please try again in a minute.";
          setGenerationError(message);
          toast.error(message);
          setMessages((prev) => prev.slice(0, -1));
          return;
        ...
        const message =
          err instanceof Error
            ? err.message
            : "Generation is temporarily unavailable. Please try again in a moment.";
        setGenerationError(message);
        toast.error(message);
        setMessages((prev) => prev.slice(0, -1));
      } finally {
